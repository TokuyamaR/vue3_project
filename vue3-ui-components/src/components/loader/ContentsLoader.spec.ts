...
describe('ContentsLoader will be displayed', () => {
  ...
  test('One long loader and one half loader will be displayed', () => {
    const wrapper = wrapperFactory({ line: 2 });
    expect(wrapper.findAll('[data-test="long-loader"]').length).toBe(1);
    expect(wrapper.findAll('[data-test="half-loader"]').length).toBe(1);
  });

  test('One half loader will be always displayed', () => {
    const wrapper = wrapperFactory({ line: 3 });
    expect(wrapper.findAll('[data-test="long-loader"]').length).toBe(2);
    expect(wrapper.findAll('[data-test="half-loader"]').length).toBe(1);
  });

  test('One half loader will be always displayed', () => {
    const wrapper = wrapperFactory({ line: 5 });
